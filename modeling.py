...
class MomentModel(nn.Module):
    ...
    def __init__(self, n_frames=-1, asr_dim=-1, args=None):
        ...
        if self.use_asr:
            self.asr_enc_layer = nn.Sequential(
                nn.LayerNorm(asr_dim), nn.Linear(asr_dim, embed_dim)
            )
        ...
        model_state_dict = torch.load(
            "./pretrained_weights/clip4caption_vit-b-32_model.bin", map_location="cpu"
        )
        ...
        self.clip4cap_model = CaptionGenerator.from_pretrained(
            "bert-base-uncased",
            "visual-base",
            "decoder-base",
            cache_dir=cache_dir,
            ...
            task_config=args,
            max_position_embeddings_override=2048,
        )
        ...
        self.clip_model, self.clip_preprocess = build_eva_model_and_transforms(
    ...
    def foward_moment_shared(
        ...
        if video_mask is None:
            video_mask = torch.ones(
                (B, max_n_frames), device=video_feats.device, dtype=torch.long
            )
        ...
        for n_frames in n_frames_batch:
            ...
            normzlied_time = torch.cat([normalized_time, padding]).view(
                1, max_n_frames, 1
            )
        ...
        # if feats.shape != temporal_embed.shape:
        #     zero_tensor = torch.zeros((1, 1, 512), device=temporal_embed.device)
        #     temporal_embed = torch.cat((temporal_embed, zero_tensor), dim=1)

        feats += temporal_embed

        mask_embed = self.mask_embed(moment_mask)

        # if feats.shape != mask_embed.shape:
        #     zero_tensor = torch.zeros((1, 1, 512), device=mask_embed.device)
        #     mask_embed = torch.cat((mask_embed, zero_tensor), dim=1)
        ...
        feats = self.clip4cap_model.get_visual_output(
            feats, torch.zeros((B, max_n_frames)).long().to(feats.device), shaped=True
        )

        return feats

    def forward_moment_retrieval(
        self, video_feats, text_feat, video_mask=None, moment_mask=None, asr_feats=None
    ):
    ...
    def train_moment_retrieval(self, batch):
        ...
        start_loss = F.binary_cross_entropy_with_logits(
            start_logits, _start_target, reduction="none"
        )
        end_loss = F.binary_cross_entropy_with_logits(
            end_logits, _end_target, reduction="none"
        )
    ...
    def forward_moment_segmentation(
        self,
        video_feats,
        text_feat,
        video_mask,
    ...
    def train_moment_segmentation(self, batch):
        ...
        moment_segmentation_logits[moment_mask == 0] = -torch.finfo(
            moment_segmentation_logits.dtype
        ).max
        moment_segmentation_loss = F.cross_entropy(
            moment_segmentation_logits, moment_segmentation_target
        )
    ...
    def test_moment_segmentation(self, batch, threshold=0.15, **kwargs):
        ...
        moment_mask = torch.zeros(
            B, video_feats.shape[1], device=video_feats.device, dtype=torch.long
        )
        for b in range(B):
            moment_mask[
                b, moment_start_boundaries[b] : moment_last_boundaries[b] + 1
            ] = 1

        prev_boundary_mask = torch.zeros(
            B, video_feats.shape[1], device=video_feats.device, dtype=torch.long
        )
        for b in range(B):
        ...
        for b in range(B):
            step_predictions[b].append(
                [moment_start_boundaries[b], moment_start_boundaries[b]]
            )
        ...
        for i in range(n_max_iteration):
            ...
            for b in range(B):
                ...
                moment_mask[
                    b, current_step_prediction[0] : current_step_prediction[1] + 1
                ] = 0
        ...
        for b in range(B):
            step_predictions[b].append(
                [moment_last_boundaries[b], moment_last_boundaries[b]]
            )
    ...
    def train_step_captioning(self, batch):
        ...
        input_caption_ids = pairs_input_caption_ids.view(
            -1, pairs_input_caption_ids.shape[-1]
        )
        ...
        visual_output = self.foward_moment_shared(
            video_feats,
            text_feat,
            video_mask=torch.ones((B, self.args.max_frames))
            .long()
            .to(video_feats.device),
            moment_mask=torch.ones((B, self.args.max_frames))
            .long()
            .to(video_feats.device),
            asr_feats=asr_feats,
        )

        decoder_scores, res_tuples = self.clip4cap_model._get_decoder_score(
            visual_output.to(video_feats.device),
            v_mask,
            input_caption_ids.long().to(video_feats.device),
            decoder_mask.long().to(video_feats.device),
            shaped=True,
        )

        pairs_output_caption_ids = (
            pairs_output_caption_ids.view(-1, pairs_output_caption_ids.shape[-1])
            .long()
            .to(video_feats.device)
        )

        loss = self.clip4cap_model.decoder_loss_fct(
            decoder_scores.view(-1, self.clip4cap_model.bert_config.vocab_size).to(
                video_feats.device
            ),
    ...
    def trim_feats(self, visual_output, moment_mask, B, device):
        ...
        for i in range(B):
            ...
            if self.args.max_frames < z.shape[0]:
                ...
                count_embeds = [
                    count_embeds[
                        (j * self.args.max_frames)
                        // N : ((j + 1) * self.args.max_frames)
                        // N
                    ]
                    for j in range(N)
                ]
    ...
    def test_step_captioning(self, batch, **kwargs):
        ...
        visual_output_rpt = visual_output.repeat(1, beam_size, 1).view(
            n_inst * beam_size, len_v, v_h
        )
        video_mask_rpt = video_mask.repeat(1, beam_size).view(n_inst * beam_size, len_v)

        inst_dec_beams = [
            Beam(beam_size, device=device, tokenizer=self.tokenizer)
        ...
        for len_dec_seq in range(1, self.args.max_words + 1):
            ...
            (visual_output_rpt, video_mask_rpt), inst_idx_to_position_map = (
                collate_active_info(
                    (visual_output_rpt, video_mask_rpt),
                    inst_idx_to_position_map,
                    active_inst_idx_list,
