    ...
    def build_loaders(self):
        ...
        if args.train:
            ...
            for task in self.tasks:
                loader = get_moment_loader(
                    args, split="train", batch_size=args.train_batch_size, task=task
                )
    ...
    def train(self):
        if args.train:
            ...
            for epoch in range(epochs):
                ...
                if "moment_retrieval" in self.tasks:
                    val_moment_retrieval_results = self.evaluate(
                        self.val_moment_retrieval_loader, has_target=True
                    )
                    val_loss += val_moment_retrieval_results["loss"]

                if "moment_segmentation" in self.tasks:

                    val_moment_segmentation_results = self.evaluate(
                        self.val_moment_segmentation_loader, has_target=False
                    )

                if "step_captioning" in self.tasks:
                    val_step_captioning_results = self.evaluate(
                        self.val_step_captioning_loader, has_target=True
                    )
                ...
                if self.verbose:
                    ...
                    if "moment_retrieval" in self.tasks:
                        print(
                            f'Moment Retrieval loss: {val_moment_retrieval_results["loss"]:.4f}'
                        )

                    if "step_captioning" in self.tasks:
                        print(
                            f'Step Captioning loss: {val_step_captioning_results["loss"]:.4f}'
                        )
                    ...
                    if "moment_retrieval" in self.tasks:
                        moment_retrieval_dump_path = os.path.join(
                            self.args.ckpt_dir,
                    ...
                    if "moment_segmentation" in self.tasks:
                        moment_segmentation_dump_path = os.path.join(
                            self.args.ckpt_dir,
                    ...
                    if "step_captioning" in self.tasks:
                        step_captioning_dump_path = os.path.join(
                            self.args.ckpt_dir,
        ...
        if args.end_to_end:
            ...
            if "moment_retrieval" in self.tasks:
                moments = self.evaluate(
                    self.test_moment_retrieval_loader, has_target=False
                )

                moment_retrieval_dump_path = os.path.join(
                    self.args.ckpt_dir, "test_moment_retrieval.json"
                )
                with open(moment_retrieval_dump_path, "w") as f:
                ...
                shutil.copyfile(
                    f"{args.data_dir}/all_data_test.json", f"{args.data_dir}/temp1.json"
                )
                ...
                with open(f"{args.data_dir}/temp1.json", "r") as f:
                    ...
                    for prompt in test:
                        ...
                        for video in test[prompt]:
                            ...
                            test[prompt][video]["bounds"] = moments[prompt][video][
                                "bounds"
                            ]
            ...
            if "moment_segmentation" in self.tasks:
                moments = self.evaluate(
                    self.test_moment_segmentation_loader, has_target=False
                )

                moment_segmentation_dump_path = os.path.join(
                    self.args.ckpt_dir, "test_moment_segmentation.json"
                )
                with open(moment_segmentation_dump_path, "w") as f:
                    json.dump(moments, f, indent=4)

                shutil.copyfile(
                    f"{args.data_dir}/all_data_test.json", f"{args.data_dir}/temp2.json"
                )
            ...
            if "step_captioning" in self.tasks:
                moments = self.evaluate(
                    self.test_step_captioning_loader, has_target=False
                )

                step_captioning_dump_path = os.path.join(
                    self.args.ckpt_dir, "test_step_captioning.json"
                )
                with open(step_captioning_dump_path, "w") as f:
                    json.dump(moments, f, indent=4)
                        ...
                        for video in test[prompt]:
                            if video in moments:
                                for i, sent in enumerate(moments[video]["captions"]):
                                    test[prompt][video]["steps"][i]["heading"] = sent[
                                        "sentence"
                                    ]

                with open(f"{self.args.ckpt_dir}/final_results.json", "w") as f:
                    json.dump(test, f, indent=2)
                    print(
                        f"Final results saved too: {self.args.ckpt_dir}/final_results.json !"
                    )

                shutil.move(
                    f"{args.data_dir}/all_data_test.json", f"{args.data_dir}/temp3.json"
                )
            ...
            if "moment_retrieval" in self.tasks:
                test_moment_retrieval_results = self.evaluate(
                    self.test_moment_retrieval_loader, has_target=False
                )
            if "moment_segmentation" in self.tasks:
                test_moment_segmentation_results = self.evaluate(
                    self.test_moment_segmentation_loader, has_target=False
                )
            if "step_captioning" in self.tasks:
                test_step_captioning_results = self.evaluate(
                    self.test_step_captioning_loader, has_target=False
                )
            ...
            if self.verbose:
                if "moment_retrieval" in self.tasks:
                    moment_retrieval_dump_path = os.path.join(
                        self.args.ckpt_dir, f"test_moment_retrieval_BEST.json"
                    )
                    with open(moment_retrieval_dump_path, "w") as f:
                ...
                if "moment_segmentation" in self.tasks:
                    moment_segmentation_dump_path = os.path.join(
                        self.args.ckpt_dir, f"test_moment_segmentation_BEST.json"
                    )
                    with open(moment_segmentation_dump_path, "w") as f:
                ...
                if "step_captioning" in self.tasks:
                    step_captioning_dump_path = os.path.join(
                        self.args.ckpt_dir, f"test_step_captioning_BEST.json"
                    )
                    with open(step_captioning_dump_path, "w") as f:
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(
                tqdm(
                    loader,
                    ncols=120,
                    desc=f"{task.capitalize()} Prediction",
            ...
            if self.args.distributed:
                ...
                if self.verbose:
                    print(
                        "after all_gather - len(predictions)",
            ...
            if tasks[0] == "moment_retrieval":
                ...
                for i in range(len(results["video_fnames"])):
                    ...
                    start = frame_index_to_timestamp(
                        raw_prediction[0],
                        results["video_duration"][i],
                        n_frames=self.args.n_model_frames,
                    )
                    end = frame_index_to_timestamp(
                        raw_prediction[1],
                        results["video_duration"][i],
                        n_frames=self.args.n_model_frames,
                    )

                    moment_retrieval_results[prompt][video_fname]["bounds"] = [
                        start,
                        end,
                    ]
                ...
                for i in range(len(results["tasks"])):
                    ...
                    moment_segmentation_results[video_fname]["target_bounds"] = results[
                        "targets"
                    ][i]
                ...
                for i in range(len(results["tasks"])):
                    ...
                    if has_target:
                        ...
                        if (
                            "target_captions"
                            not in step_captioning_results[video_fname]
                        ):
                            step_captioning_results[video_fname]["target_captions"] = []

                        step_captioning_results[video_fname]["target_captions"].append(
                            target_text
                        )
